...
// ルートシグネチャ設定
HRESULT DepthMapIntegration::CreateRootSignature()
{
    //サンプラー作成
    ...
    //ディスクリプタテーブルのスロット設定
    descTableRange[0].Init(D3D12_DESCRIPTOR_RANGE_TYPE_SRV, 1, 0); // shadowmap1
    descTableRange[1].Init(D3D12_DESCRIPTOR_RANGE_TYPE_SRV, 1, 1); // shadowmap2
...
// シェーダー設定
HRESULT DepthMapIntegration::ShaderCompile()
{
    std::string cs = "DepthMapIntegration.hlsl";
...
// パイプラインの生成
HRESULT DepthMapIntegration::CreatePipeline()
{
...
// ヒープの生成
HRESULT DepthMapIntegration::CreateHeap()
{
    D3D12_DESCRIPTOR_HEAP_DESC desc{};
    desc.Flags = D3D12_DESCRIPTOR_HEAP_FLAG_SHADER_VISIBLE;
    desc.NodeMask = 0;
    desc.NumDescriptors = 4; // shadowmap*2, output, copy dst
...
// 出力用テクスチャリソースの生成
HRESULT DepthMapIntegration::CreateTextureResource()
{
...
// ビューの生成
void DepthMapIntegration::CreateView()
{
...
// 実行
void DepthMapIntegration::Execution(ID3D12CommandQueue* _cmdQueue, ID3D12CommandAllocator* _cmdAllocator, ID3D12GraphicsCommandList* _cmdList)
{
    //それぞれのセット
...
void DepthMapIntegration::ChangeResolution(int _width, int _height)
{
    width = _width;
    height = _height;
