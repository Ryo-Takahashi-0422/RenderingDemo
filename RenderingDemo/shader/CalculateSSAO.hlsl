RWTexture2D<float4> ssao; // UAVÉIÉuÉWÉFÉNÉg
...
void cs_main(uint3 DTid : SV_DispatchThreadID)
{
    ...
    if (dp < 1.0f)
    {
        for (int i = 0; i < trycnt; ++i)
        {
            ...
            // óêêîÇÃåãâ ñ@ê¸ÇÃîΩëŒë§Ç…å¸Ç¢ÇƒÇ¢ÇΩÇÁîΩì]
            float dt = dot(norm, omega);
            float sgn = sign(dt);
            omega *= sign(dt);
            dt *= sgn; // ê≥ÇÃílÇ…ÇµÇƒcosÉ∆ÇìæÇÈ            
            ...
            //float depthDifference = sampleDepth - rpos.z;
            ao += step(sampleDepth + 0.0005f, rpos.z) * dt * normDiff/* * (1.0 - smoothstep(0.000002f, 0.0007f, depthDifference))*/;
        }
